...
id= 1
path = 'b'
# calibInputImage(readImage, cameraMatrix, Distortion_Parameters, path, id)
...
while True:
    readImage= cv.imread('D:\\Project\\b\\calibresult1.jpg')
    rect= findContour(readImage)[0]
    thre= findContour(readImage)[1]
    calculatorLength(32, 1920, rect, thre, readImage, writeLength= True)
    thre = np.stack((thre, )*3, axis=-1)
    img = np.hstack((readImage, thre))
    img = cv.resize(img, (0,0), fx=0.25, fy=0.25)

    cv.imshow('namespace', img)
